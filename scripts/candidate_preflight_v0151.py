...
from proworksim.candidate_runtime_v0151 import CandidateActor, candidate_profile, inspect_chat_stop
from proworksim.online_training import probability_check, reference
...
def run_preflight(args, *, actor_factory=None):
    ...
    closed = False
    replay_records = []
    replay_paths = list(getattr(args, "replay_call", None) or [])
    if len(replay_paths) > 2:
        raise ValueError("At most two predeclared real source prompts may be replayed")
    stage = 'load'
    report = {'version': 'candidate-preflight-v0.15.1', 'scope':
    ...
    try:
        ...
        closed = True
        if replay_paths:
            stage = 'original_public_request_replay'
            closed = False
            owner.begin_window('original-public-request-chat-stop-calibration')
            for index, source_path in enumerate(replay_paths):
                source = read_json(Path(source_path))
                replay_request = source['request']
                rendered, _, _ = owner.prepare_request(replay_request)
                current_ids = owner.tokenizer(rendered, add_special_tokens=False)['input_ids']
                original_ids = source['response']['token_trace']['input_ids']
                record = {'source_call': reference(source_path),
                    'source_response_id': source['response'].get('id'),
                    'source_input_ids_sha256': digest(json_bytes(original_ids)),
                    'current_input_ids_equal_source': current_ids == original_ids,
                    'sampling_seed': recipe['seed'] + 101 + index,
                    'scope': 'New generation on the exact recorded public prompt; no world action or old-token repair'}
                replay_records.append(record)
                if current_ids != original_ids:
                    raise ValueError('Original native prompt IDs changed; replay is not performed')
                owner.reseed(record['sampling_seed'], label=f'original-prompt-{index}')
                response = owner.complete(replay_request, timeout_seconds=600)
                calls.append(response)
                record.update(http_status=response['http_status'],
                    new_response_id=response['body'].get('id'),
                    stop_check=inspect_chat_stop(response['body'], owner.inference_profile))
            owner.finish_evaluation([], args.output / 'closed-original-prompt-window')
            closed = True
    ...
    stop_checks = [inspect_chat_stop(call.get('body', {}), owner.inference_profile)
                   for call in calls if owner is not None and isinstance(call, dict)
                   and call.get('http_status') == 200]
    protocol_ready = bool(stop_checks) and all(row['passed'] for row in stop_checks)
    replay_ready = len(replay_records) == len(replay_paths) and all(
        row.get('http_status') == 200 and row.get('stop_check', {}).get('passed')
        and row['current_input_ids_equal_source'] for row in replay_records)
    report['generation_observed'] = bool(real_traces)
    report['inference_ready'] = bool(real_traces) and protocol_ready and replay_ready
    report.update(chat_stop_checks=stop_checks, chat_stop_protocol_ready=protocol_ready,
        requested_replay_calls=len(replay_paths), replay_records=replay_records,
        original_prompt_replay_ready=replay_ready,
        native_chat_end_observed=any(row['native_end_present'] for row in stop_checks))
    if owner is not None and closed and report['inference_ready']:
        try:
        ...
        backward['skip_reason'] = 'No complete interface boundary with verified raw chat termination'
...
def main():
    ...
    parser.add_argument('--replay-call', type=Path, action='append', default=[],
                        help='At most two fixed original resident call records; new sampling, no world execution')
