...
from proworksim.templates.learning_work import build_learning_case, registry, witness_code
...
class PublicWitnessPolicy:
    ...
    def decide(self, context):
        ...
        for route in obs["information_routes"]:
            if route["provider"] != role or role in route["recipients"] or task not in {"handoff", "chain"}:
                continue
        ...
        if task == "handoff" or role == "provider":
            return done()
        if role == "implementer":
            if task == "chain" and self.config["communication"] == "requested" and "request" not in cache:
            ...
            if "written" not in cache:
                code = witness_code(wrong=control == "wrong_sql", old_rules=control == "old_rules")
        ...
        if role == "reviewer":
            ...
            transactions = records(cache["read:data"]["data"]["tables"]["transactions"])
            rows = records(cache["read:result"]["data"]["tables"]["metrics"])
            errors = []
            # Local public evidence computation is separate from domain evaluator.
            for index, row in enumerate(rows):
                eligible = [t for t in transactions if t["customer_id"] == row["customer_id"]
                            and t["period"] == audit["period"] and t["status"] in audit["allowed_statuses"]
                            and audit["order_min_inclusive"] <= t["order_id"] < audit["order_max_exclusive"]]
                revenue = sum(t["amount"] for t in eligible) * audit["amount_factor"]
                count = len(eligible) if audit["deduplication"] == "transaction_rows" else len({t["order_id"] for t in eligible})
                for column, actual, expected in [(1, row["revenue_cents"], revenue), (2, row["order_count"], count)]:
                    if actual != expected:
                        errors.append((index, column))
            if not errors or control == "bad_approval":
                return act("judgment", "approve", work_id=wid, submission_id=sid)
            index, column = (3, 2) if control == "wrong_location" else errors[0]
...
def run_case(case_id, output, *, communication="proactive", control=None, variant="v13"):
    output = Path(output)
    prepared = build_learning_case(case_id, output)
...
def main():
    ...
    for case in registry()["situations"]:
        modes = ["proactive", "requested"] if case["task"] == "chain" else ["proactive"]
        for mode in modes:
    ...
    for case_id, control, expected in [
        ("development-v14-f0-implement", "old_rules", 0.2),
        ("development-v14-f1-implement", "wrong_sql", 0.2),
        ("development-v14-f0-review", "bad_approval", 0.25),
        ("development-v14-f0-review", "wrong_location", 0.25),
        ("development-v14-f1-review", "no_actions", 0),
        ("development-v14-f0-handoff", "read_only", 0.25),
    ]:
    ...
    report = {"version": "learning-work-feasibility-v0.14", "model_execution": False,
