...
[研究目标与理论设计 V1](docs/research/id-vtdo.md)是研究入口；当前按[v0.16 计划](docs/harness-v016-plan.md)独立开发真实 SDK 工作人员，并保留[v0.15 原线](docs/learning-v015-plan.md)的冻结筛选与有限在线学习执行图，ID-VTDO 支持随在线窗口诊断。基础 RL 不要求先取得完整成功或多类方法支持，不使用 SFT 主路线；贡献调权与强基线比较另按实际支持条件开展。

WorldCore管理权限、版本、采用、工作义务和问题处理；工作人员决定业务动作；场景控制器执行预声明事件。机构接受、内容正确、未知、服务故障和训练奖励分别记录。

## v0.16 SDK 工作人员与独立实验（进行中）
...
真实 9B H0 已完成3例、33次生成、0次参数更新：接口校准接通；实现任务正确构建但未提交（R=0.5）；双成员未完成交接（R=0）。尚无 harness 收益比较。H1 两模型48例目前为不可直接执行的规划，等待原 S1 收口与正式证据绑定；H2 尚未启动。完整失败、资源、SDK表示修订与协议描述勘误均保留。

## v0.15 真实来源与候选模型实验（进行中）
...
旧7B已完成36例筛选，完整责任完成0/36，分项成果主要来自读取/交接。9B原运行暴露聊天EOS错误，旧记录保留且退出能力比较；[新停止协议](docs/design/candidate-chat-stop-v0151.md)已通过实际校准，正在重新执行冻结筛选。27B权重下载与校验已完成，通过短S0后已进入原冻结S1。候选尚未全部测量，N0和MC/RTG新在线比较未启动，当前不能声称模型排名或学习收益。
