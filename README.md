...
[研究目标与理论设计 V1](docs/research/id-vtdo.md)是研究入口；当前按[v0.15 计划](docs/learning-v015-plan.md)推进候选模型选型、真实公开来源与有限在线学习比较，ID-VTDO 支持随在线窗口诊断。基础 RL 不要求先取得完整成功或多类方法支持，不使用 SFT 主路线；贡献调权与强基线比较另按实际支持条件开展。
...
旧7B已完成36例筛选，完整责任完成0/36，分项成果主要来自读取/交接。9B原运行暴露聊天EOS错误，旧记录保留且退出能力比较；[新停止协议](docs/design/candidate-chat-stop-v0151.md)已通过实际校准，正在重新执行冻结筛选。27B权重下载与校验并行进行。候选尚未全部测量，N0和MC/RTG新在线比较未启动，当前不能声称模型排名或学习收益。
