...
class ScenarioDeployment:
    ...
    def prepare_start(self, runtime, controller=None):
        ...
        self.prefix = {
            "prefix_executed": True,
            "status": "unbuildable",
            "reason": "Declared prefix target was not reached within its bound",
            "opportunities": results,
        }
...
def bind_runtime(deployment, checkpoint=None, recorder=None, captured=None):
    ...
    for role in deployment.role_bindings():
        ...
        policies[label] = factories[role["policy"]](**role["config"])
    return StaffRuntime(ports, policies, checkpoint=checkpoint, recorder=recorder)


def run_scenario(deployment, runtime=None, controller=None):
    ...
    for _ in range(bound["max_opportunities"]):
        ...
        if condition and not remaining and not deployment.world.state.get("events"):
            status = "completed"
