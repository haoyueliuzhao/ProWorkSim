...
def _policy_identity(policy):
    return {"implementation": type(policy).__module__ + "." + type(policy).__qualname__,
            "config": copy.deepcopy(getattr(policy, "config", {}))}


class StaffRuntime:
    def __init__(self, ports, policies, checkpoint=None, run_id=None, recorder=None):
        if (not isinstance(ports, dict) or not ports or not isinstance(policies, dict) or set(ports) != set(policies)
        ...
        if saved is not None:
            if (saved.get("version") != RUNTIME_VERSION or saved.get("labels") != self.labels
                    or saved.get("policy_identities") != identities):
                raise ValueError("Checkpoint runtime, labels/order and policy configuration must match")
        ...
        self.roles = saved["roles"] if saved else {
            label: {"memory": {}, "last_action": None, "last_result": None,
                    "status": "ready", "actions": 0, "opportunities": 0, "identity": None}
            for label in self.labels
        }
        previous_events = saved["experience"]["events"] if saved else None
        if recorder is not None and previous_events is not None and recorder.events != previous_events:
    ...
    def snapshot(self):
        ...
        return copy.deepcopy({"version": RUNTIME_VERSION, "run_id": self.run_id,
                              "labels": self.labels, "policy_identities": self.policy_identities,
                              "actions": self.actions, "opportunities": self.opportunities,
                              "cursor": self.cursor, "roles": self.roles,
    ...
    def _step(self):
        ...
        context = {"worker_id": label, "tools": copy.deepcopy(definitions),
                   "observation": copy.deepcopy(observation), "memory": copy.deepcopy(role["memory"]),
        ...
        try:
            ...
            json_bytes(decision)
            if (not isinstance(decision, dict) or decision.get("kind") not in {"act", "wait", "done"}
            ...
            if decision["kind"] == "act" and (
                not isinstance(decision.get("action"), str) or not decision["action"]
        ...
        public_decision = {key: copy.deepcopy(value) for key, value in decision.items() if key != "memory"}
        self.recorder.record("policy_decision", {
            "decision": public_decision, "memory_before_sha256": digest(json_bytes(role["memory"])),
        ...
        if decision["kind"] != "act":
            ...
            return self._finish(label, status, decision.get("reason", decision["kind"]),
        ...
        try:
            response = self.ports[label].call(action["action"], request_key=key, **action["arguments"])
        ...
        if not isinstance(response, dict) or type(response.get("ok")) is not bool:
            result = self._finish(label, "environment_error", "Public tool returned no explicit result")
        elif response["ok"]:
            result = self._finish(label, "running", "Actual policy action returned successfully")
        else:
            failure = classify_tool_result(response)
            result = self._finish(label, failure["status"], "Actual tool refusal preserved", rejection=failure)
        return {**result, "action_performed": True, "response": copy.deepcopy(response),
                "decision": public_decision}
    ...
    def run(self, max_actions=100, max_opportunities=300):
        ...
        while self.actions - start_actions < max_actions and self.opportunities - start_opportunities < max_opportunities:
            outcome = self.step()
            ...
            if len(idle) == len(self.labels):
                ...
                break
        boundary = {"run_id": self.run_id, "status": status,
        ...
        return {**boundary, "total_actions": self.actions, "total_opportunities": self.opportunities,
                "roles": copy.deepcopy(self.roles), "outcomes": outcomes,
                "experience": self.recorder.snapshot()}
