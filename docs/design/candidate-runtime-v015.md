...
S0 报告将两种准入明确分开：`inference_ready` 只依据至少一次真实200响应及非空原始输入／输出token；`training_ready` 还要求窗口封口、所有已生成序列的概率复算、可微路径同阈值核对、有限非零反向梯度和完整检查点保存重载全部通过。数值或序列化异常保留分阶段错误、完整日志与部分测量，允许S0作为已完成测量退出0，但不能标记训练可行。加载失败或没有真实生成退出1并禁止进入S1。两种情况均写报告。S1从原始固定检查点重新加载，不沿用S0状态；工具任务本身是否完成由后续能力筛选另测。
