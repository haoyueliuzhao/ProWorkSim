...
日期：2026-09-27，v0.16 模型—harness—世界组合与归因修订。以用户提供的[数据需求增强版理论稿](../reference/id-vtdo-v1-data-design.md)为完整设计依据，以[v0.14审计及模型建议](../reference/learning-audit-v014-model-selection.md)与[v0.15 harness审计](../reference/learning-audit-v015-harness.md)及[当前计划](../harness-v016-plan.md)规定本轮优先级；[v0.15原计划](../learning-v015-plan.md)继续按冻结合同执行。本文是该设计与仓库实现、实验门槛之间的正式对应；不把设计稿中的数量算例写成已建数据或运行结果。原ProWorkSim版本报告保留为历史证据。
...
## v0.15：初始化能力、真实来源与算法效应分开

初始策略不必是厂商命名的纯预训练Base；可选择公开后训练检查点。候选选型检验的是初始工作能力及训练集成成本，不是本项目的学习收益。所有正式对照从选定同一检查点及相同LoRA/critic/optimizer状态开始，所有岗位仍共享一套有效actor参数。选型不采本项目教师轨迹、不做针对性SFT，不为不同岗位混用大小模型。

当前重点从绑定旧Qwen2.5-7B转为实测少量候选：Qwen3.8-27B、Qwen3.5-9B，Qwen3-32B作为事先登记的集成备选。架构、原生工具模板、思考模式、采样分布与精度均属Γ，不能仅改模型名称后沿用旧概率公式或attention补丁。模板正确、可微概率可复算与工作完成分别核验；某个候选的加载/数值失败不等于其业务能力差。多卡设备占用、前向/反向、开发评价与失败重试成本一并记账。

真实UCI Online Retail成为新的直接操作对象；真实发票/取消编码/数量/时间/单价/客户字段与模拟报告政策分别公开。输入按完整发票和客户关联划分用途，参数训练、选型开发、同源锁定不交叉；所有派生责任保留来源链。54条CPU合法路径只证明有限情境可行，不成为目标模型经验。单岗位实现与复核、双成员信息获取、三成员完整链使用同一真实数据环境；机构接受、业务正确和过程资格仍分开。

Marshmallow维护资产提供不同工作类型的可执行对象：代码编辑、实际隔离测试和固定提交。模拟需求不是原项目真实缺陷历史；单个新需求也不等于整个软件维护基准。私有判据在可信父进程核对隔离子进程返回的实际API值/异常，防止提交代码猴补unittest造成假通过；它不因此变为对任意恶意程序的完整形式化证明。

本轮先执行开发能力筛选，再以计划大小的两窗N0验证新数值路线，最后冻结小型MC/RTG pilot及全部训练后测量。把有限pilot直接放到新UCI来源，避免来源接入永远等待构造SQL显著收益。Marshmallow不用于选型和参数/外层更新，只作预声明初始/最终有限评价；公开原项目可能在预训练出现，不能保证污染为零。

模型A比旧7B好是选型结果；更新后比自身初始好是基础学习变化；同起点同任务同预算下ID-VTDO优于强基线才是算法增量。新来源数量、当前ξ方法支持和多seed复现也互不替代。原v0.14缺失的训练后评价继续标未完成；本轮候选权重下载、CPU规则和工程预检不提前记作完整学习比较结果。

## v0.16：将工作人员harness纳入固定协议Γ

模型能力不是与运行系统无关的标量。本轮旧S1测量的是 `U_dev(θ_m, Γ0)`，其机械selected只表示当前候选和集成条件下的相对选择，不构成足够工作能力的统一认证。各岗位、事实与重复结果仍须共同呈现。OpenHands SDK 1.49.6 的独立开发不热替换Γ0或旧四窗轨迹。

新比较分别估计固定θ0的 `U(θ0, ΓH)-U(θ0, Γ0)`，以及固定ΓH的 `U(θT, ΓH)-U(θ0, ΓH)`。前者是harness组合效果，后者才是参数学习变化；二者均不是ID-VTDO增量。新Γ显式包含SDK/模板、回合调度、私有笔记/待办、局部检索、上下文选择、编辑语法、真实工具执行器、停止和预算。程序维护调用身份、版本谱系和字节记录，业务来源选择、SQL内容、采用和复核判断仍由模型承担。

H0实际兼容验收后才能冻结H1组合；H1只使用独立客户/发票的UCI开发素材，不使用Marshmallow锁定需求改提示或工具。模型输出、同事输入和工具返回严格分开：额外note/检索动作保存真实自己生成token，历史被移出本次输入则明确记录选择，不能重新分词整理文本当作原样本。角色记忆不等于世界事实，可读权限不等于实际查询，程序为编辑检查当前元数据也不等于模型已在输入里看到它。

共享模型、现有learner/critic、FP32/highest和LoRA范围保持。混合模型仅full-attention q/v适配，DeltaNet冻结；实际模块及参数量单独记录，相同rank不代表相同容量。H2重新固定(θ0,ΓH)后才研究学习，旧Γ0结果保持独立；当前θ/Γ/ξ内重新统计支持，不跨harness并池、不将机械检索误标为自主信息获取方法。没有支持保持Q=B，不以框架升级自动启动O4。

四项目关系扩展并行推进；有限程序见证和独立来源探针只支撑各自范围，不作为已经完成参数学习或广泛工作迁移的替代证据。
