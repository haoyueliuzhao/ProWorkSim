...
所有实验保存版本、实际工具动作、来源和失败。本轮不进行参数更新，不重新改写历史 API 轨迹。详细结果见 [实验报告](experiments/world-semantics-v03.md)。

## 固定模型失败复核后的 v0.3.1 补修

首轮冻结的 7 个 DeepSeek 情境有 6 个完成且独立通过；unavailable 最终合理阻塞，但此前两次提交缺适用批准的成果，均被拒绝。失败轨迹还暴露了两处世界语义缺口，已在本轮继续修正：

1. 依据适用性按当前产物对应的工作阶段、分析需求版本、逻辑节点及经理确认核对，不只比较依据文件的全局最新版本。缺当前确认时为 unknown；合法的既有确认不因无关依据新版本出现而自动失效。工作激活和需求替换完成后立即刷新这些派生状态。
2. 没有适用确认时，不从隐藏生成规格中借用数值目标。评价仍拒绝缺依据交付，仍检查公开披露及依赖；依赖确认的数值目标记为 unassessed_checks，numerical_assessment 为 blocked_missing_approved_basis，不冒充通过或数值错误。评价器独立升级至 operating-world-v0.3.1，公开内容合同仍为 operating-world-v0.3，schema仍为0.3。

原 API 运行、文件、调用、观察和首轮评价保留。scripts/reevaluate_world_staff.py 只在独立副本刷新诊断并追加新版评价，保存原文件/调用/消息/审阅的前后哈希；不重新运行工作人员、不向原模型提供新状态、不修改业务终态。补修后整套回归为 142 项通过。
